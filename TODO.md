...
- [ ] **T7.2：实现 workspace/session browser。** 支持 workspace list、create、reorder、archive、ungrouped、session list、selected/running/blank 状态和空会话复用。
  - 进度：`NativeWorkspaceAuthorityTests` 已覆盖 archive receipt 的 authority 边界：Host 接受 `workspace.archiveSession` 后 browser 仍保持最后完整 list snapshot，不乐观加入 archived ID 或移除 session；只有下一次 `workspace.list`/`session.list` Host refresh 才发布新 workspace/session 列表。create/reorder/归档完整操作、浏览器视觉与 macOS evidence 仍待完成，故保持未勾选。
  - 依赖：T4.5、T6.1、T7.1。
