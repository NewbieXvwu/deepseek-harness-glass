# 工程反模式案例册 (Engineering Casebook)

> 本文件是 [`AGENTS.md`](../AGENTS.md) 的配套案例册：章程只保留可执行的硬规则，这里保留每条规则的**真实事故现场与正反代码对比**，供实现或审查时查阅。
>
> 规则本身以 `AGENTS.md` 为准；本文件不新增规则，只解释规则从哪来。
...
#### 真实对比案例：`glass/Tests/Core/GhostPlaneModuleManifestTests.swift`
...
#### 真实对比案例：`glass/Sources/PluginPlane/GhostPlaneTemporaryFileStore.swift`
...
#### 真实对比案例：`glass/Sources/PluginPlane/GhostPlaneWebViewHost.swift`
...
#### 真实对比案例：`glass/ci/check-authenticated-host-fixtures.py`
...
#### 真实对比案例：`tools/spec-generation/generate_official_remote_contract_manifest.py` 与 `tools/spec-generation/extract_official_remote_contract_ast.mjs`
...
#### 真实对比案例：`glass/ci/check-official-remote-contract.py`
