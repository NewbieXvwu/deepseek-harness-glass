...
| P1 | `check-accessibility-baseline.py` | 原按 fixture 读取 Swift source path 是否存在，现已删除。 | `OfficialAccessibilityBaselineCatalog` 从实际 GlassSpec resource bundle decode baseline；`OfficialUISpecBuildTests` 验证 RC8 commit、六条 scene、环境 contract、runtime official label resolution 与 unknown scene/unregistered label 负例。受 TCC trust 的 GUI host 继续运行真实 accessibility-tree/keyboard tests。 | **已完成 source-gate 删除与 runtime baseline contract 迁移，GUI accessibility automation 仍待 T12.8 闭环**；不得恢复 source path 检查。 |
