...
| P1 | `check-official-locale-literals.py` | 原通过 `Text("...")` 等源码正则判定 UI 文案，现已删除。 | `NativeAccessibilityRuntimeTests` 实际装载 Composer/SwiftUI control，读取 native accessibility tree 的 visible labels 并和 `OfficialLocaleCatalog.values` runtime catalog 比对；测试以 injected non-official Button label 负例证明 catalog 可拒绝。 | **已完成实现迁移，待当前提交 macOS CI**；不得恢复源码 literal regex。`check-official-spec.py` 的其他源码文本规则仍独立审计/迁移。 |
