...
const seededFixture = join(process.cwd(), 'snapshots/web/seeded-history/session.jsonl')
...
async function capture(page: Page, tripwire: ReturnType<typeof watchConsole>): Promise<void> {
  const name = 'details-closed-and-reopen'
  ...
  await writeFile(join(outputDirectory, `${name}.json`), JSON.stringify({
    ...
    colorScheme: 'light',
    geometry,
    detailsTrack: await detailsTrack(page),
    ariaSnapshot,
...
describe('reference capture: rc.1 CUT1.8 details lifecycle', () => {
  ...
  it('captures the recovered Session with the details track still closed', async () => {
    const scaffold = await launchWebScaffold({
      replayFixture: lifecycleFixture,
      paceMs: 5,
      compareReplaySession: false,
    })
    await seedSession(scaffold, await readFile(seededFixture, 'utf8'), 'details-session-lifecycle-seed')
