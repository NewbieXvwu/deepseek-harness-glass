...
struct HostRuntimeLadderPortableCheck {
    static func main() throws {
        let ladder = HostRuntimeLadder(lockedBuildID: "dsh-v0.1.1-rc.2")
        ...
        try equal(ladder.select(attach: attach, adopt: adopt, install: install), .attach(endpoint: URL(string: "http://127.0.0.1:7331/")!, access: .readOnly(reason: "Attached Host build local-dev is not locked build dsh-v0.1.1-rc.2.")), "attach priority")
