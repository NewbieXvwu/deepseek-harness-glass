...
@main
struct HostRuntimeStaticInstallationScannerPortableCheck {
    static func main() throws {
        ...
        precondition(HostRuntimeStaticInstallationScanner().scan(resourcesRoots: [root, root]).count == 1)
        try FileManager.default.removeItem(at: catalog)
        precondition(HostRuntimeStaticInstallationScanner().scan(resourcesRoots: [root]).isEmpty)
    }
}
