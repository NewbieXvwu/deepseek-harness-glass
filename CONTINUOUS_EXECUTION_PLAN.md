...
| 5.6 | T11.5–T11.7 | 仅在兼容矩阵批准后完成登记制 `GlassPluginPlane` host、single-card 可嵌入性实验和零核心 WebView 不变量。资源与 boot graph admission 已实施；官方任意 `tapIndex` callback 已收缩为 graph `(pluginID, revision)` 绑定、仅允许 token/data/class 的纯 Core replay plan，尚未在 WebKit 应用。下一步须以参数化调用接入该 plan，并补齐官方 ModuleLoader/SlotRegistry、typed injection gate、response MIME/redirect 与绿色区路由。所有外站、file URL、未知 scheme、popup 与全局 session UI 必须被拒绝。 |
