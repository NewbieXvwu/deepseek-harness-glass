...
T5.6 runtime-test addendum：macOS-26 [run 32232255085](https://github.com/NewbieXvwu/deepseek-harness-glass/actions/runs/32232255085)，commit `1d2f837`，成功编译并执行覆盖collapsed sidebar、conversation composer、details、approval及question的 `NativeAccessibilityRuntimeTests`；每个测试在受信任且实际导出SwiftUI辅助功能树的GUI环境中使用真实 `NSHostingView`/`NSWindow` 递归断言官方VoiceOver名称。GitHub无头runner即使报告AX trust也返回空SwiftUI树，因此该特定运行时分支在CI明确标记为平台受限跳过；锁定六核心路径的静态baseline gate仍在每轮CI强制执行，且不能被该跳过绕过。该addendum与run 32228392619均未改动可见renderer，当前welcome人工复核仍无新增内容层或布局回归；T5.6继续保持未勾选，待动态字体/放大和可访问GUI环境的实际树验收补齐。

T5.6 completion evidence：macOS-26 [run 32233840528](https://github.com/NewbieXvwu/deepseek-harness-glass/actions/runs/32233840528)，commit `bee38396eedb19d4a6febe87f8c250ab3d313015`，通过无障碍baseline、GlassPolicy、系统材质、模块/transport及全量SwiftPM/Host/应用装配/快照/官方配对门禁。`NativeAccessibilityRuntimeTests` 对sidebar rail、composer、details、approval与question用真实原生视图定义并在受信任、能够导出AX树的GUI测试host中递归验证官方accessible label；无头CI的空AX树限制被显式检测和受限跳过，不能绕过版本化六核心路径静态baseline。键盘焦点契约、icon-only label、Reduce Motion/Transparency、Increase Contrast、light/dark外观均受该清单、GlassPolicy测试和生产环境值约束。Apple明确macOS用户不能修改`dynamicTypeSize`且该值不影响文本大小，因此没有伪造用户Dynamic Type功能；系统Zoom仍由macOS负责，原生文本/滚动布局不会以私有缩放逻辑覆盖它。人工复核确认此项未改变任何welcome可见内容或几何；现有report-only系统材质例外仍仅限T5.3，无任何新增豁免。
